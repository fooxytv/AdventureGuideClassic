<#
.SYNOPSIS
    Installs Adventure Guide Classic straight from GitHub into a WoW AddOns
...
.DESCRIPTION
    ...
    report "1.7.1" are indistinguishable in game, and by the time something
    behaves oddly you are debugging the wrong one.
...
.PARAMETER Branch
    Branch to install. Defaults to main. Any branch, tag or commit that GitHub
    will serve an archive for works.
...
.PARAMETER All
    Install to every WoW client found. The addon ships for Classic Era and for
    TBC/Anniversary, and leaving them on different builds is how you end up
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\Install-AdventureGuide.ps1

    Installs main, asking once which client if there is more than one.

.EXAMPLE
    .\Install-AdventureGuide.ps1 -Branch claude/my-feature -All

    Installs a feature branch to every client at once. The addon list will read
    1.7.1-my-feature.<sha> in each.

.EXAMPLE
    .\Install-AdventureGuide.ps1 -Flavour era

    Installs main to the Classic Era client only.
...
.NOTES
    ...
    file once:  Unblock-File .\Install-AdventureGuide.ps1
#>
...
$AddonName  = 'AdventureGuideClassic'   # must match the .toc basename
...
#
# tools/ is the Python generator scripts and their .tsv sources; todo.md is
# developer notes. Neither is addon content. (package.sh does not exclude them
# yet -- see the ticket that added this script.)
$ExcludeDirs = @('.git', '.github', 'ci', 'tools', 'docs', 'code', 'dist', '.vscode', '.claude')
$ExcludeFiles = @('README.md', 'CLAUDE.md', 'todo.md', '.luacheckrc', '.gitignore', '.gitattributes')
...
function Get-FlavourLabel([string] $Folder) {
    ...
    switch -Regex ($Folder) {
        ...
        '^_classic_$'         { 'Classic progression (Anniversary / TBC)' ; break }
...
function Resolve-Targets {
    ...
    # Every client at once. The reason this exists: the addon ships for Classic
    # Era and for TBC/Anniversary, and keeping them on different builds is how
    # you end up testing the wrong one.
...
try {
    ...
    foreach ($toc in Get-ChildItem -LiteralPath $extracted.FullName -Filter '*.toc') {
        ...
        # [^\r\n]* rather than .*$ so a CRLF line keeps its carriage return --
        # the .toc files in this repo are CRLF, and .NET's `.` matches \r.
