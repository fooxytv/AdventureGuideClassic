...
This matters more here than it would in a normal addon. Every file begins with
`select(2, ...).SetupGlobalFacade()`, which setfenv's the file into a table
whose __index falls through to _G -- so a mistyped API name does not raise, it
quietly resolves to nil and the failure surfaces later as a missing icon or a
nil compare. .luacheckrc is the list of names that are allowed to resolve that
way, and this checks the source against it.
